...
public abstract class IndexReader {
  ...
  protected SynchronizedBitSet deletedDocuments = null;
  protected int deletedOffset = 0;
  ...
  public static IndexReader open(String path) throws IOException {
    return open(FSDirectory.getDirectory(path, false), true, false);
  }
  ...
  public static IndexReader open(final Directory directory) throws IOException {
    return open(directory, false, false);
  }

  /** Returns an IndexReader reading the index in the given Directory. */
  public static IndexReader open(final Directory directory, boolean deletedFilters) throws IOException {
    return open(directory, false, deletedFilters);
  }

  private static IndexReader open(final Directory directory, final boolean closeDirectory, boolean deletedFilters) throws IOException {
      synchronized (directory) {		// in- & inter-process sync

          SegmentInfos infos;
          SynchronizedBitSet sbs = null;
          SegmentInfos fsSegmentInfos = IndexObjectsFactory.getFSSegmentInfos(directory);
          synchronized (fsSegmentInfos) {
              if (fsSegmentInfos.size() < 1) {
              ...
              infos = fsSegmentInfos.copy();
              int infossize = infos.size();
              
              DeletedInfos delInfos = IndexObjectsFactory.getDeletedInfos(directory);
              if (delInfos != null && !deletedFilters) {
                  sbs = delInfos.getBitSet();//delInfos.copyBitSet();
              } else {
            	  sbs = new SynchronizedBitSet();
              }
              
              if (infossize == 1) {		  // index is optimized
                  SegmentReader reader = SegmentReader.get(infos, infos.info(0), closeDirectory);
                  reader.setDeletedDocuments(sbs);
                  return reader;
              }
              ...
              for (int i = 0; i < infossize; i++) {
                  ...
                  readers[i].setDeletedDocuments(sbs);
                  readers[i].setDeletedOffset(currentOffset);
                  currentOffset += readers[i].maxDoc();
              }
              return new MultiReader(directory, infos, sbs, closeDirectory, readers);          
          }
      }
  }
  ...
  public void setDeletedDocuments(SynchronizedBitSet deletedDocuments) {
      this.deletedDocuments = deletedDocuments;
  }
  
  public void setDeletedOffset(int deletedOffset) {
   ...
   */
  /*
   * REMOVED to allow for the subclassing of FSDirectory
   */
   ...
   */
  /*
   * REMOTED because SEGMENTS has moved to a relational db
   */
